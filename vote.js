...
voteRouter.use(async (req, res, next) => {
  ...
  voteRouter.post('/create', async (req, res, next) => {
    let { options, ...body } = req.body
    try {
      let vote = await Vote.create(body)
      vote.setUser(req.user)//当前登陆用户
  
      let ary = await Promise.all(options.map(str => {
...
voteRouter.post('/voteup/:optionId', async (req, res, next) => {
  let option = await Option.findByPk(req.params.optionId, { include: Vote })
  if (option) {//选项存在
    if (option.Vote.deadline.getTime() > Date.now()) {//未过期
      if (option.Vote.desc) {//多选
